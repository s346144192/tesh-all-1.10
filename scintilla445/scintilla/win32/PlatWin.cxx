...
FontID CreateFontFromParameters(const FontParameters &fp) {
	...
	if (fp.technology == SCWIN_TECH_GDI) {
		...
		HRESULT hr = pIDWriteFactory->CreateTextFormat(wsFace.c_str(), nullptr,
			...
			style,
			DWRITE_FONT_STRETCH_NORMAL, fHeight, L"zh-Hans", &pTextFormat);//L"zh-Hans",L"en-us"
		if (SUCCEEDED(hr)) {
...
bool Window::SetPositionRelative(PRectangle rc, const Window *relativeTo, bool limit_rect) {
	...
	if (style & WS_POPUP) {
		...
		if (limit_rect) {
			RECT rect = { 0,0,0,0 };
			::GetWindowRect(HwndFromWindow(*relativeTo), &rect);
			if (static_cast<LONG>(rc.left) < rect.left
				|| static_cast<LONG>(rc.top) < rect.top
				|| static_cast<LONG>(rc.right) > rect.right
				|| static_cast<LONG>(rc.bottom) > rect.bottom) {
				return false;
			}
		}
...
bool Window::IsShow() {
	return IsWindowVisible(HwndFromWindowID(wid));
}
...
public:
	...
	void Update(int ctrlID_, Point location_, int lineHeight_, bool unicodeMode_, int technology_) override;
...
void ListBoxX::Create(Window &parent_, int ctrlID_, Point location_, int lineHeight_, bool unicodeMode_, int technology_) {
	/*if (Created()) {
		Update(ctrlID_, location_, lineHeight_, unicodeMode_, technology_);
		return;
	}*/
	parent = &parent_;
	ctrlID = ctrlID_;
	...
	//窗口边界设置 AdjustWindowRect(&rcDesired, dpi);
	wid = ::CreateWindowEx(
...
void ListBoxX::Update(int ctrlID_, Point location_, int lineHeight_, bool unicodeMode_, int technology_) {
	if (!parent || !Created()) {
		return;
	}
	ctrlID = ctrlID_;
...
void ListBoxX::SetRedraw(bool on) noexcept {
	...
	if (on)
		::InvalidateRect(lb, nullptr, FALSE);
}
