...
public:
	MessageCommandCallback* pMessageCommand;
	MessageNotifyCallback* pMessageNotify;
...
sptr_t ScintillaWin::MouseMessage(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case WM_LBUTTONDOWN: {
			...
			//	KeyboardIsKeyDown(VK_MENU));
			::SetFocus(MainHWND());
			int modifiers = MouseModifiers(wParam);
			ButtonDownWithModifiers(PointFromLParam(lParam), ::GetMessageTime(),
				modifiers);
			SCNotification scn = {};
			scn.nmhdr.code = WM_LBUTTONDOWN;
	...
	case WM_LBUTTONUP:
	{
		...
		ButtonUpWithModifiers(PointFromLParam(lParam),
			::GetMessageTime(), modifiers);
		SCNotification scn = {};
		scn.nmhdr.code = WM_LBUTTONUP;
		scn.modifiers = modifiers;
		NotifyParent(scn);
	}
		break;
	...
	case WM_RBUTTONDOWN: {
			...
			int modifiers = MouseModifiers(wParam);
			RightButtonDownWithModifiers(pt, ::GetMessageTime(), modifiers);
			SCNotification scn = {};
			scn.nmhdr.code = WM_RBUTTONDOWN;
			scn.modifiers = modifiers;
			NotifyParent(scn);
		}
		break;
...
sptr_t ScintillaWin::KeyMessage(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case WM_CHAR:
		if (( ((wParam >= 128) || !lastKeyDownConsumed ) && !iscntrl(static_cast<int>(wParam)) )) {
...
void ScintillaWin::NotifyChange() {
	if (pMessageCommand) {
		pMessageCommand((sptr_t)MainHWND(), GetCtrlID(), SCEN_CHANGE);
		return;
	}
...
void ScintillaWin::NotifyFocus(bool focus) {
	if (commandEvents) {
		if (pMessageCommand) {
			pMessageCommand((sptr_t)MainHWND(), GetCtrlID(), focus ? SCEN_SETFOCUS : SCEN_KILLFOCUS);
			return;
		}
		::SendMessage(::GetParent(MainHWND()), WM_COMMAND,
...
void ScintillaWin::NotifyParent(SCNotification scn) {
	...
	if (scn.nmhdr.code== SCN_UPDATEUI) {
		if (scn.updated & SC_UPDATE_V_SCROLL  || scn.updated & SC_UPDATE_H_SCROLL) {
			AutoCompleteUpdate();
			CallTipUpdate();
		}
	}
	if (pMessageNotify) {
		pMessageNotify((sptr_t)MainHWND(), &scn);
		return;
	}
...
void ScintillaWin::NotifyDoubleClick(Point pt, int modifiers) {
	...
	SCNotification scn = {};
	scn.nmhdr.code = WM_LBUTTONDBLCLK;
	scn.modifiers = modifiers;
	NotifyParent(scn);
}
...
namespace Scintilla {
//新增
	void SetCallbackMessageCommand(ScintillaWin* sci, MessageCommandCallback* callback) {
		sci->pMessageCommand = callback;
	}
	void SetCallbackMessageNotify(ScintillaWin* sci, MessageNotifyCallback* callback) {
		sci->pMessageNotify = callback;
	}

	void _cdecl SetBraceLight(ScintillaWin* sci, int stpos, int endpos) {
		if (sci) {
			if (endpos >= 0) {
				sci->SetBraceHighlight(static_cast<Sci::Position>(stpos), endpos, STYLE_BRACELIGHT);
			}
			else {
				sci->SetBraceHighlight(static_cast<Sci::Position>(stpos), -1, STYLE_BRACEBAD);
			}
		}
	}
	int _cdecl SetBraceMatch(ScintillaWin* sci, int pos, int length) {
		if (sci) {
			return sci->pdoc->BraceMatch(static_cast<Sci::Position>(pos), length, 0, false);
		}
		return -1;
	}
	
	void _cdecl StyleSetMessage(ScintillaWin* sci, unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
		if (sci) {
			sci->StyleSetMessage(iMessage, wParam, lParam);
		}
	}
	sptr_t _cdecl StyleGetMessage(ScintillaWin* sci, unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
		if (sci) {
			return sci->StyleGetMessage(iMessage, wParam, lParam);
		}
		return 0;
	}

	int _cdecl GetStyleAt(ScintillaWin* sci, int i) {
		if (sci) {
			if (static_cast<Sci::Position>(i) >= sci->pdoc->Length())
				return 0;
			else
				return sci->pdoc->StyleAt(static_cast<Sci::Position>(i));
		}
		return 0;
	}
	...
	sptr_t _cdecl GetIndicStart(ScintillaWin* sci ,sptr_t indicator, sptr_t pos) {
		if (sci) {
			return sci->pdoc->decorations->Start(static_cast<int>(indicator), pos);
		}
		return 0;
	}
	sptr_t _cdecl GetIndicEnd(ScintillaWin* sci, sptr_t indicator, sptr_t pos) {
		if (sci) {
			return sci->pdoc->decorations->End(static_cast<int>(indicator), pos);
		}
		return 0;
	}
	sptr_t _cdecl GetIndicAtValue(ScintillaWin* sci, sptr_t indicator, sptr_t pos) {
		if (sci) {
			return sci->pdoc->decorations->ValueAt(static_cast<int>(indicator), pos);
		}
		return 0;
	}
	sptr_t _cdecl GetCurrentIndicValue(ScintillaWin* sci) {
		if (sci) {
			return sci->pdoc->decorations->GetCurrentValue();
		}
		return 0;
	}
	void _cdecl SetCurrentIndic(ScintillaWin* sci, sptr_t indicator) {
		if (sci) {
			sci->pdoc->DecorationSetCurrentIndicator(static_cast<int>(indicator));
		}
	}
	void _cdecl SetCurrentIndicValue(ScintillaWin* sci, sptr_t value) {
		if (sci) {
			sci->pdoc->decorations->SetCurrentValue(static_cast<int>(value));
		}
	}
	void _cdecl SetIndicRange(ScintillaWin* sci, sptr_t start, sptr_t length) {
		if (sci) {
			sci->pdoc->DecorationFillRange(static_cast<Sci::Position>(start),sci->pdoc->decorations->GetCurrentValue(), length);
		}
	}
	void _cdecl SetIndicRangeExt(ScintillaWin* sci, sptr_t indicator, sptr_t start, sptr_t length) {
		if (sci) {
			SetCurrentIndic(sci, indicator);
			SetIndicRange(sci, start,length);
		}
	}
	void _cdecl SetIndicClearRange(ScintillaWin* sci, sptr_t start, sptr_t lengthClear) {
		if (sci) {
			sci->pdoc->DecorationFillRange(static_cast<Sci::Position>(start), 0, lengthClear);
		}
	}


	int _cdecl GetWordStartPos(ScintillaWin* sci, int pos) {
		if (sci) {
			if (!sci->pdoc) {
				return -1;
			}
			return sci->pdoc->ExtendWordSelect(static_cast<Sci::Position>(pos), -1, FALSE);
		}
		return -1;
	}
	int _cdecl GetWordEndPos(ScintillaWin* sci, int pos) {
		if (sci) {
			return  sci->pdoc->ExtendWordSelect(static_cast<Sci::Position>(pos), 1, FALSE);
		}
		return -1;
	}
	int _cdecl GetTextRange(ScintillaWin* sci, sptr_t lParam) {
		if (sci) {
			return sci->GetTextRangeExt(lParam);
		}
		return 0;
	}

	sptr_t _cdecl SetText(ScintillaWin* sci,const char* text) {
		if (sci) {
			return sci->SetTextExt(text);
		}
		return 0;
	}
	int _cdecl GetText(ScintillaWin* sci,sptr_t length, char* buffer) {
		if (sci) {
			return sci->GetTextExt(length,buffer);
		}
		return 0;
	}

	int _cdecl GetTextLength(ScintillaWin* sci) {
		if (sci) {
			return sci->pdoc->Length();
		}
		return 0;
	}
	int _cdecl GetLineCount(ScintillaWin* sci) {
		if (sci) {
			if (sci->pdoc->LinesTotal() == 0)
				return 1;
			else
				return sci->pdoc->LinesTotal();
		}
		return 1;
	}
	int _cdecl GetLineFromPos(ScintillaWin* sci, int i) {
		if (sci) {
			if (!sci->pdoc || static_cast<Sci::Position>(i) < 0)
				return 0;
			return sci->pdoc->LineFromPosition(static_cast<Sci::Position>(i));
		}
		return 0;
	}
	int _cdecl GetLineStartPos(ScintillaWin* sci, int i) {
		if (sci) {
			if (static_cast<Sci::Position>(i) < 0)
				i = sci->pdoc->LineFromPosition(sci->SelectionStart().Position());
			if (i == 0)
				return 0; 	// Even if there is no text, there is a first line that starts at 0
			if (static_cast<Sci::Line>(i) > sci->pdoc->LinesTotal())
				return -1;
			//if (wParam > pdoc->LineFromPosition(pdoc->Length()))	// Useful test, anyway...
			//	return -1;
			return sci->pdoc->LineStart(static_cast<Sci::Position>(i));
		}
		return -1;
	}
	int _cdecl GetLineEndPos(ScintillaWin* sci, int i) {
		if (sci) {
			return sci->pdoc->LineEnd(static_cast<Sci::Line>(i));
		}
		return 0;
	}
	int _cdecl GetLineLength(ScintillaWin* sci, int i) {
		if (sci) {
			if ((static_cast<Sci::Position>(i) < 0) ||
				(static_cast<Sci::Position>(i) > sci->pdoc->LineFromPosition(sci->pdoc->Length())))
				return 0;
			return sci->pdoc->LineStart(static_cast<Sci::Position>(i) + 1) - sci->pdoc->LineStart(static_cast<Sci::Position>(i));
		}
		return 0;
	}

	char _cdecl GetCharAt(ScintillaWin* sci, int i) {
		if (sci) {
			return sci->pdoc->CharAt(i);
		}
		return 0;
	}
	int _cdecl GetCurrentPos(ScintillaWin* sci) {
		if (sci) {
			return sci->sel.IsRectangular() ? sci->sel.Rectangular().caret.Position() : sci->sel.MainCaret();
		}
		return -1;
	}

	void _cdecl EnsureStyledAll(ScintillaWin* sci) {
		if (sci) {
			return sci->EnsureStyledAll();
		}
	}
	void _cdecl EnsureCaretVisible(ScintillaWin* sci) {
		if (sci) {
			return sci->EnsureCaretVisible();
		}
	}
	int _cdecl GetFoldLevel(ScintillaWin* sci, int line) {
		if (sci) {
			return sci->pdoc->GetLevel(static_cast<Sci::Line>(line));
		}
		return SC_FOLDLEVELBASE;
	}
	int _cdecl GetFoldParent(ScintillaWin* sci, int line) {
		if (sci) {
			return sci->pdoc->GetFoldParent(static_cast<Sci::Line>(line));
		}
		return -1;
	}
	BOOL _cdecl GetFoldExpanded(ScintillaWin* sci, int line) {
		if (sci) {
			return sci->pcs->GetExpanded(static_cast<Sci::Line>(line));
		}
		return FALSE;
	}
	void _cdecl SetFoldLine(ScintillaWin* sci, int line, int action) {
		if (sci) {
			sci->FoldLine(static_cast<Sci::Line>(line), static_cast<int>(action));
		}
	}
	void _cdecl SetFoldLineNotUpdate(ScintillaWin* sci, int line, int action) {
		if (sci) {
			sci->FoldLineNotUpdate(static_cast<Sci::Line>(line), static_cast<int>(action));
		}
	}
	void _cdecl SetFoldAll(ScintillaWin* sci, int action) {
		if (sci) {
			sci->FoldAll(static_cast<int>(action));
		}
	}
	void _cdecl SetRedraw(ScintillaWin* sci) {
		if (sci) {
			sci->SetRedraw();
		}
	}
	int _cdecl GetAnchor(ScintillaWin* sci) {
		if (sci) {
			return sci->sel.IsRectangular() ? sci->sel.Rectangular().anchor.Position() : sci->sel.MainAnchor();
		}
		return -1;
	}
	int GetSelectionStart(ScintillaWin* sci) {
		if (sci) {
			return sci->sel.LimitsForRectangularElseMain().start.Position();
		}
		return 0;
	}
	int GetSelectionEnd(ScintillaWin* sci) {
		if (sci) {
			return sci->sel.LimitsForRectangularElseMain().end.Position();
		}
		return 0;
	}
	size_t SetSearchStringRanges(ScintillaWin* sci,const char* str, Sci::Position start
		, Sci::Position end, int SearchFlags, int indic, int indicvalue) {
		if (sci) {
			return sci->SetSearchStringRanges(str,start,end, SearchFlags, indic, indicvalue);
		}
		return 0;
	}
	int GetSearchStringPosStart(ScintillaWin* sci, size_t i) {
		if (sci) {
			if (i< sci->SearchStringRanges.size()) {
				return sci->SearchStringRanges.at(i).start;
			}
		}
		return -1;
	}
	int SelectSearchString(ScintillaWin* sci, size_t i) {
		if (sci) {
			return sci->SelectSearchString(i);
		}
		return -1;
	}
	//替换删除指定的搜索范围,返回替换字符串长度。
	int ReplaceSearchString(ScintillaWin* sci, size_t i, const char* newstr, bool select) {
		if (sci) {
			return sci->ReplaceSearchString(i,newstr,select);
		}
		return 0;
	}
	//替换自动清理当前搜索范围,返回替换数量。
	size_t ReplaceAllSearchString(ScintillaWin* sci, const char* newstr, bool select) {
		if (sci) {
			return sci->ReplaceAllSearchString(newstr, select);
		}
		return 0;
	}
	size_t GetSearchStringsCount(ScintillaWin* sci) {
		if (sci) {
			return sci->SearchStringRanges.size();
		}
		return 0;
	}
	void SetsSearchStringsClear(ScintillaWin* sci ) {
		if (sci) {
			sci->SetsSearchStringsClear();
		}
	}
	bool AutoCompleteActive(ScintillaWin* sci) {
		if (sci) {
			return sci->AutoCompleteActive();
		}
		return false;
	}
	void AutoCompleteShow(ScintillaWin* sci, Sci::Position lenEntered, const char *list) {
		if (sci) {
			sci->AutoCompleteShow(lenEntered, list);
		}
	}
	void AutoCompleteUpdate(ScintillaWin* sci) {
		if (sci) {
			sci->AutoCompleteUpdate();
		}
	}
	void AutoCompleteCancel(ScintillaWin* sci) {
		if (sci) {
			sci->AutoCompleteCancelExt();
		}
	}
	void CallTipUpdate(ScintillaWin* sci) {
		if (sci) {
			sci->CallTipUpdate();
		}
	}
