...
int Editor::KeyCommand(unsigned int iMessage) {
	...
	case SCI_FORMFEED:
		...
		break;
	case SCI_ZOOMIN:
		if (vs.zoomLevel < vs.zoomLevelMax) {
			vs.zoomLevel++;
			InvalidateStyleRedraw();
			NotifyZoom();
		}
		break;
	case SCI_ZOOMOUT:
		if (vs.zoomLevel > vs.zoomLevelMin) {
			vs.zoomLevel--;
			InvalidateStyleRedraw();
			NotifyZoom();
		}
 ...
 */
void Editor::SearchAnchor(int idx) {
	if (idx > 0) {
		searchAnchor = idx;
	}
	else if (idx == -1)
	{
		...
		searchAnchor = pdoc->Length();
	}
	else if (idx == -3) {
		searchAnchor = SelectionStart().Position();
	}
	else if (idx == -4) {
		searchAnchor = SelectionEnd().Position();
	}
}
...
void Editor::FoldLineNotUpdate(Sci::Line line, int action) {
	if (line >= 0) {
...
Sci::Position Editor::ReplaceTarget(bool replacePatterns, const char *text, Sci::Position length) {
	...
	if (targetRange.start.Position() <0 || targetRange.end.Position() < 0) {
		return 0;
	}
...
void Editor::SetPosFoldExpandVisible(Sci::Position pos) {
	...
	do {
		FoldLineNotUpdate(line, SC_FOLDACTION_EXPAND);
...
size_t Editor::UpdateSearchStringRanges(Sci::Position start, Sci::Position end) {
	...
	for (;;) {
		...
		try {
			const Sci::Position pos = pdoc->FindText(searchStart, searchEnd,
				CurSearchString.data(), CurSearchStrFlag, &lengthFound);
			if (pos >= 0 && lengthFound > 0) {
				SearchStringRanges.push_back(Range(pos, pos + lengthFound));
				searchStart = pos + lengthFound;
			}
...
int Editor::ReplaceSearchString(size_t i, const char* cstr, bool select) {
	...
	if (i < SearchStringRanges.size()) {
		...
		int len= ReplaceTarget(CurSearchStrFlag & SCFIND_REGEXP || CurSearchStrFlag & SCFIND_CXX11REGEX,
...
size_t Editor::ReplaceAllSearchString(const char* cstr, bool select) {
	...
	for (; SearchStringRanges.size();) {
		...
		len = ReplaceTarget(CurSearchStrFlag & SCFIND_REGEXP || CurSearchStrFlag & SCFIND_CXX11REGEX,
			newstr.data(), static_cast<Sci::Position>(newstr.length()));
...
sptr_t Editor::GetTextRangeExt(sptr_t lParam) {
	if (lParam == 0)
		return 0;
...
int Editor::GetTextExt(int length, char* buffer) {
	if (!pdoc) {
		return 0;
	}
	if (buffer == nullptr)
	...
	if (length == 0)
		return 0;
	char *ptr = buffer;
	const Sci_Position len = std::min<Sci_Position>(length - 1, pdoc->Length());
	pdoc->GetCharRange(ptr, 0, len);
	ptr[len] = '\0';
	return len;
}
int Editor::SetTextExt(const char* text) {
	if (!pdoc || text == nullptr)
		return 0;
	UndoGroup ug(pdoc);
	pdoc->DeleteChars(0, pdoc->Length());
	SetEmptySelection(0);
...
sptr_t Editor::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case SCI_SEARCHANCHOR:
		SearchAnchor(static_cast<int>(wParam));
		break;
	...
	case SCI_SETZOOMLEVELMINMAX:
		vs.zoomLevelMin = wParam;
		vs.zoomLevelMax = lParam;
		break;
	default:
