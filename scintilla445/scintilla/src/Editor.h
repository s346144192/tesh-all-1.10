...
public:
	...
	//=================新增=================
	Range SelectSearchRange;
	std::vector<Range> SearchStringRanges;
	int SearchStringLight; //绑定
	int SearchStringLightValue; //绑定
	std::string CurSearchString; //绑定
	int CurSearchStrFlag; //绑定

	void SetRedraw();
	int GetTextExt(int length, char* buffer);
	int SetTextExt(const char* text);
	void FoldLineNotUpdate(Sci::Line line, int action);
	void FoldLine(Sci::Line line, int action);
	void FoldAll(int action);
	void EnsureCaretVisible(bool useMargin = true, bool vert = true, bool horiz = true);
	void SetBraceHighlight(Sci::Position pos0, Sci::Position pos1, int matchStyle);
	...
	SelectionPosition SelectionStart();
	SelectionPosition SelectionEnd();
	sptr_t GetTextRangeExt(sptr_t lParam);
	void EnsureStyledAll();

	void SetPosFoldExpandVisible(Sci::Position pos);

	void SetSearchStringLight(size_t i,bool isClear);

	void SetSearchStringLight(bool isClear);
	size_t SetSearchStringRanges(const char* str, Sci::Position start, Sci::Position end, int flags,int indic=-1, int indicvalue = 0);
	size_t UpdateSearchStringRanges(Sci::Position start, Sci::Position end);
	int SelectSearchString(size_t i);
	int ReplaceSearchString(size_t i,const char* newstr,bool select=true);
	size_t ReplaceAllSearchString(const char* newstr, bool select = true);
