...
void ScintillaBase::AutoCompleteStart(Sci::Position lenEntered, const char *list) {
	...
	ac.lb->SetPositionRelative(rcList, &wMain,false);
	ac.Show(true);
	if (lenEntered != 0) {
...
void ScintillaBase::AutoCompleteUpdate() {
	...
	const PRectangle rcClient = GetClientRectangle();
	Point pt = LocationFromPosition(sel.MainCaret() - ac.startLen);
	PRectangle rcPopupBounds = wMain.GetMonitorRect(pt);
	if (rcPopupBounds.Height() == 0)
		rcPopupBounds = rcClient;
	...
	//if (pt.y >= rcPopupBounds.bottom - heightLB &&  // Won't fit below.
	//	pt.y >= (rcPopupBounds.bottom + rcPopupBounds.top) / 2) { // and there is more room above.
	//	if (pt.y - heightLB < rcPopupBounds.top) {
	//		heightLB -= static_cast<int>(rcPopupBounds.top - (pt.y - heightLB));
	//	}
	//}
	...
	bool rt=ac.lb->SetPositionRelative(rcList, &wMain,true);
	//if (!rt) {
		ac.Show(rt);
	//}
}
...
void ScintillaBase::AutoCompleteCancelExt() {
	ac.Cancel();
}
...
void ScintillaBase::CallTipShow(/*Point pt,*/ Sci::Position pos, const char *defn) {
	ac.Cancel();
	...
	ct.wCallTip.SetPositionRelative(rc, &wMain,false);
	ct.wCallTip.Show();
}

void ScintillaBase::CallTipUpdate() {
	if (!ct.inCallTipMode) {
		return;
	}
	Point pt = LocationFromPosition(ct.posWordStart);
	PRectangle rc = ct.GetRectangleExt(pt,vs.lineHeight);
	const PRectangle rcClient = GetClientRectangle();
	const int offset = vs.lineHeight + static_cast<int>(rc.Height());
	// adjust so it displays above the text.
	if (rc.bottom > rcClient.bottom && rc.Height() < rcClient.Height()) {
	...
	// Now display the window.
	CreateCallTipWindow(rc);
	bool rt=ct.wCallTip.SetPositionRelative(rc, &wMain,true);
	//if (ct.wCallTip.IsShow() != rt) {
		ct.wCallTip.Show(rt);
	//}
}
...
sptr_t ScintillaBase::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case SCI_CALLTIPSHOW:
		CallTipShow(static_cast<Sci::Position>(wParam),
