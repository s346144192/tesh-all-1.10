...
void LineMarker::Draw(Surface *surface, PRectangle &rcWhole, Font &fontForCharacter, FoldPart part, int marginStyle) const {
	...
	case SC_MARK_PLUS: {
		//正方形边框
		//colourTail.SetRGB(153, 153, 153);
		//colourTail.SetRGB(fore.GetRed() / 2 * 3, fore.GetGreen() / 2 * 3, fore.GetBlue() / 2 * 3);
		//DrawBox(surface, centreX, centreY, blobSize + 1, colourHead, colourTail);

		DrawBox(surface, centreX, centreY, blobSize, colourHead, fore);

		//DrawBox(surface, centreX, centreY, blobSize - 1, colourHead, colourTail);
		//+
		//加深
		//colourTail.SetRGB(102, 102, 102);
		/*colourTail.SetRGB(fore.GetRed() / 3 * 4, fore.GetGreen() / 3 * 4, fore.GetBlue() / 3 * 4);
		DrawPlus(surface, centreX + 1, centreY, blobSize - 1, colourTail);
		DrawPlus(surface, centreX - 1, centreY, blobSize - 1, colourTail);
		DrawPlus(surface, centreX, centreY - 1, blobSize - 1, colourTail);
		DrawPlus(surface, centreX, centreY + 1, blobSize - 1, colourTail);*/
		DrawPlus(surface, centreX, centreY, blobSize - 1, fore);
		}
		break;
	...
	case SC_MARK_MINUS: {
		...
		//渐变
		//colourTail.SetRGB(fore.GetRed() / 3 * 4, fore.GetGreen() / 3 * 4, fore.GetBlue() / 3 * 4);
		//DrawMinus(surface, centreX, centreY+1, blobSize - 1, colourTail);
		DrawMinus(surface, centreX, centreY, blobSize - 1, fore);
		//DrawMinus(surface, centreX, centreY-1, blobSize - 1, colourTail);
		}
		break;
	...
	/*		
			ColourDesired newcolor(std::min(colourBody.GetRed() / 3 * 4, 255), std::min(colourBody.GetGreen() / 3 * 4, 255), std::min(colourBody.GetBlue() / 3 * 4, 255));
			surface->PenColour(newcolor);
			
			surface->MoveTo(centreX-1 , ircWhole.top);
			surface->LineTo(centreX-1, ircWhole.bottom);
	...
	case SC_MARK_LCORNER: {
			const int offset= ((ircWhole.bottom - centreY) / 2);
			surface->PenColour(colourTail);
			surface->MoveTo(centreX, ircWhole.top);
			surface->LineTo(centreX, centreY + offset);
			surface->LineTo(ircWhole.right - 1 + offset, centreY + offset);

			//surface->MoveTo(centreX, ircWhole.top);
			//surface->LineTo(centreX, ircWhole.bottom - 1);
			//surface->LineTo(ircWhole.right - 1, ircWhole.bottom - 1);
			
			//ColourDesired newcolor(
			//	std::min(colourTail.GetRed() / 3 * 4,255),
			//	std::min(colourTail.GetGreen() / 3 * 4,255),
			//	std::min(colourTail.GetBlue() / 3 * 4,255));
			//surface->PenColour(newcolor);
			//
			////内边
			//surface->MoveTo(centreX - 1, ircWhole.top);
			//surface->LineTo(centreX - 1, ircWhole.bottom - 1 + 1);
			//surface->LineTo(ircWhole.right - 1, ircWhole.bottom - 1 + 1);
			////外边
			//surface->MoveTo(centreX + 1, ircWhole.top);
			//surface->LineTo(centreX + 1, ircWhole.bottom - 1 - 1);
			//surface->LineTo(ircWhole.right - 1, ircWhole.bottom - 1 - 1);
			////边角
			//surface->MoveTo(ircWhole.right, ircWhole.bottom - 1 - 1);
			//surface->LineTo(ircWhole.right, ircWhole.bottom - 1 + 1);
		}
		break;
	...
	case SC_MARK_TCORNER: {
			const int offset = ((ircWhole.bottom - centreY) / 2);
			surface->PenColour(colourTail);
			surface->MoveTo(centreX, centreY + offset);
			surface->LineTo(ircWhole.right - 1 + offset, centreY + offset);

			/*surface->MoveTo(centreX, ircWhole.bottom - 1);
			surface->LineTo(ircWhole.right - 1, ircWhole.bottom - 1);*/

			surface->PenColour(colourBody);
			surface->MoveTo(centreX, ircWhole.top);
			surface->LineTo(centreX, ircWhole.bottom - 1 + 1);
			...
			////水平
			//surface->MoveTo(centreX+1, ircWhole.bottom - 1 - 1);
			//surface->LineTo(ircWhole.right - 1, ircWhole.bottom - 1 - 1);
			////内边垂直
			//surface->MoveTo(centreX+1, ircWhole.top);
			//surface->LineTo(centreX+1, ircWhole.bottom - 1 - 1);
	...
	case SC_MARK_BOXPLUS: {
		...
		/*if (ircWhole.bottom > centreY + blobSize + 2) {
			surface->PenColour(fore);
			surface->MoveTo(centreX, centreY + blobSize + 2);
			surface->LineTo(centreX, ircWhole.bottom);

			surface->PenColour(colourTail);

			surface->MoveTo(centreX - 1, centreY + blobSize + 2);
			surface->LineTo(centreX - 1, ircWhole.bottom);
	...
	case SC_MARK_BOXPLUSCONNECTED: {
		...
		//DrawPlus(surface, centreX, centreY + 1, blobSize - 1, colourTail);
		DrawPlus(surface, centreX, centreY, blobSize, fore);
		if (ircWhole.top < centreY - blobSize - 1) {
			surface->PenColour(fore);
			surface->MoveTo(centreX, ircWhole.top);
			...
			//surface->MoveTo(centreX - 1, ircWhole.top);
			//surface->LineTo(centreX - 1, centreY - blobSize - 2);

			//surface->MoveTo(centreX + 1, ircWhole.top);
			//surface->LineTo(centreX + 1, centreY - blobSize - 2);
		}

		if (ircWhole.bottom > centreY + blobSize + 1) {
			surface->PenColour(fore);
